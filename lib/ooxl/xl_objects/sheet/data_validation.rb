  ...
  class Sheet
    class DataValidation
      ...
      def in_sqref_range?(cell_id)
        return false if cell_id.blank?

        cell_letter = cell_id.gsub(/\d/, '')
        index = cell_id.gsub(/[^\d]/, '').to_i
        range = sqref_range.find do |single_cell_letter_or_range, _row_range|
          if single_cell_letter_or_range.is_a?(Range)
            single_cell_letter_or_range.include?(cell_letter) || in_normalized_range?(
              single_cell_letter_or_range, cell_letter
            )
          else
            single_cell_letter_or_range == cell_letter
          end
      ...
      def build_multiple_range(sqref)
        sqref.split.to_h do |splitted_by_space_sqref|
          # ["BH5:BH271, "BI5:BI271"]
          if splitted_by_space_sqref.is_a?(Array)
            splitted_by_space_sqref.map do |_sqref|
