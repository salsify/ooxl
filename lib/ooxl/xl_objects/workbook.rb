class OOXL
  class Workbook
    ...
    def sheets
      ...
      @sheets ||= @xml.xpath('//sheets/sheet').map do |sheet_node|
    ...
    def defined_names
      @defined_names ||= @xml.xpath('//definedNames/definedName').to_h do |defined_names_node|
    ...
    def self.load_from_stream(xml_stream)
      new(Nokogiri.XML(xml_stream).remove_namespaces!)
    end
  end
end
